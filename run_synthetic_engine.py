...
def main():
    ...
    plt.title("Histogram of Pixel ADU Values (Raw CCD, Log Scale)", fontsize=13)
    ...
    if all(os.path.exists(f) for f in [cluster_map_file, adu_weighted_file, redistributed_file]):
        ...
        plt.ylabel("Normalised Intensity", fontsize=15)
        plt.xticks(fontsize=15)
        plt.yticks(fontsize=15)
        plt.title("Synthetic vs Image 8 Spectrum", fontsize=15)
